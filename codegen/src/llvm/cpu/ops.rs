...
fn codegen_loop<'ctx>(
    ...
    match uop.op() {
        ...
        Op::Bind { var, value } => {
            // Check if binding to an Outer range - generate inline loop
            if let Op::Range { end, axis_id, axis_type: AxisType::Outer } = value.op() {
                // Generate end value
                ...
                // Build loop structure using existing infrastructure
                let (loop_ctx, counter_val) =
                    loop_gen::build_loop(context, builder, function, end_int, axis_id.value() as u64)?;

                // Store loop context keyed by RANGE id (for END to find)
            ...
            // Fallback: BIND evaluates to its variable value (non-Outer case)
...
fn codegen_range<'ctx>(
    ...
    // Outer ranges: loop was generated by BIND - return counter value if available
    if axis_type == AxisType::Outer {
        // If we have a value for this range, it was generated by Bind
        if let Some(val) = values.get(uop_id) {
            return Ok(Some(val));
        }
        // Fallback: generate end value (for edge cases or legacy code paths)
...
fn codegen_end<'ctx>(
    ...
    for range_uop in ranges.iter().rev() {
        // Get loop context - works for both Outer (inlined) and Loop ranges
        // Reduce ranges don't have loop contexts (handled by REDUCE codegen)
        if let Some(loop_ctx) = values.get_loop(range_uop.id) {
...
fn codegen_reduce<'ctx>(
    ...
    // Filter to only Reduce ranges (Loop and Outer are handled by END)
    // For CPU with inlined outer loops, Outer ranges already have values from Bind
    let reduce_ranges: Vec<_> = ranges
        .iter()
        .filter(|r| if let Op::Range { axis_type, .. } = r.op() { *axis_type == AxisType::Reduce } else { false })
        .collect();
    ...
    // For single reduce range, use PHI-based approach (efficient)
    // For multiple reduce ranges, use alloca-based approach (simpler, like Tinygrad's DEFINE_REG)
    if reduce_ranges.len() == 1 {
        // Single reduce range - use PHI-based reduce loop
        let range_uop = reduce_ranges[0];
        ...
        let (header_block, exit_block, counter_val, counter_phi, acc_phi) =
            loop_gen::build_reduce_loop(context, builder, function, end_int, identity, reduce_id)?;
        ...
        let new_acc = codegen_reduce_op(reduce_op, acc_phi.as_basic_value(), src_val, result_dtype, module, builder)?;

        let final_acc =
            loop_gen::complete_reduce_loop(context, builder, header_block, exit_block, counter_phi, acc_phi, new_acc)?;

        Ok(Some(final_acc))
    } else {
        // Multiple reduce ranges - use alloca-based approach (like Tinygrad's DEFINE_REG)
        // This avoids complex nested PHI nodes
        ...
        // Build nested counting loops (all regular loops, no reduce PHIs)
        ...
        // In innermost loop body: load acc, apply reduce, store back
        ...
        // Close all loops (innermost first)
