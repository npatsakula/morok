...
fn classify_op(op: &Op) -> OpCategory {
    match op {
        ...
        // Buffer/DefineGlobal/DefineLocal/DefineVar should already be in ValueMap
        // If we encounter them during codegen, they were pre-populated by the renderer
        Op::Buffer { .. } | Op::DefineGlobal(_) | Op::DefineLocal(_) | Op::DefineVar { .. } => OpCategory::Meta,
...
fn codegen_memory<'ctx>(
    ...
    match uop.op() {
        ...
        Op::PointerIndex { ptr, offset } => {
            ...
            let result = unsafe {
                builder
                    .build_gep(context.i8_type(), ptr_val.into_pointer_value(), &[offset_val.into_int_value()], "ptr_idx")
                    .context(BuildGepSnafu)?
            };
            Ok(Some(result.into()))
        }
...
fn codegen_store<'ctx>(
    ...
    let element_ptr = if index.is_pointer_value() {
        ...
        unsafe { builder.build_gep(block.get_context().i8_type(), ptr_val, &[index_val], "gep").context(BuildGepSnafu)? }
    };
...
fn codegen_range<'ctx>(
    ...
    // Outer ranges are handled externally by the scheduler
    if axis_type == AxisType::Outer {
        return Ok(Some(require_value(end, context, module, builder, values)?));
    }

    // Reduce ranges are handled by REDUCE codegen
...
fn codegen_end<'ctx>(
    ...
    for range_uop in ranges.iter().rev() {
        ...
        let loop_ctx = values.get_loop(range_uop.id).context(LoopContextNotFoundSnafu { id: range_uop.id })?.clone();
...
fn codegen_reduce<'ctx>(
    ...
    // Handle Outer ranges by evaluating them
    ...
    // Store counter value for source computation
    values.insert(range_uop.id, counter_val.into());

    // Generate source computation
    let src_val = require_value(src, context, module, builder, values)?;
    let src_val = auto_load_pointer(src_val, &src.dtype(), context, builder)?;
    ...
    // Complete the reduce loop
    let final_acc = loop_gen::complete_reduce_loop(context, builder, header_block, exit_block, counter_phi, acc_phi, new_acc)?;
...
fn get_intrinsic<'ctx>(name: &str, ret_type: BasicValueEnum<'ctx>, module: &Module<'ctx>) -> Result<FunctionValue<'ctx>> {
    let intrinsic = Intrinsic::find(name).context(IntrinsicNotFoundSnafu { name: name.to_string() })?;
    intrinsic.get_declaration(module, &[ret_type.get_type()]).context(IntrinsicDeclarationSnafu { name: name.to_string() })
}
...
fn codegen_sign<'ctx>(src: BasicValueEnum<'ctx>, is_float: bool, builder: &Builder<'ctx>) -> Result<BasicValueEnum<'ctx>> {
    if is_float {
        let float_val = src.into_float_value();
        let zero = float_val.get_type().const_zero();
        let one = float_val.get_type().const_float(1.0);

        let is_pos = builder.build_float_compare(FloatPredicate::OGT, float_val, zero, "is_pos").context(ComparisonSnafu)?;
        let is_neg = builder.build_float_compare(FloatPredicate::OLT, float_val, zero, "is_neg").context(ComparisonSnafu)?;
...
fn codegen_reduce_identity<'ctx>(reduce_op: ReduceOp, dtype: &DType, context: &'ctx Context) -> Result<BasicValueEnum<'ctx>> {
    let llvm_type = common::dtype_to_basic_type(dtype, context)?;
