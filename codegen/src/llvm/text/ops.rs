//! LLVM IR operation rendering.
//!
//! This module generates LLVM IR strings for individual UOp operations.
...
/// Binary operation instruction lookup (Tinygrad's lop).
fn binary_instr(op: BinaryOp, dtype: &DType) -> &'static str {
    ...
    match op {
        ...
        BinaryOp::Max => {
            if is_float {
                // Will use intrinsic, not direct instruction
                "maxnum"
            } else if is_signed {
                "smax"
            } else {
        ...
        BinaryOp::Pow => "pow",      // Will use intrinsic
        BinaryOp::Threefry => "xor", // Threefry is implemented via multiple ops
    }
}

/// Unary operation instruction lookup.
fn unary_instr(op: UnaryOp, dtype: &DType) -> Option<&'static str> {
    ...
    match op {
        ...
        UnaryOp::Rsqrt => None, // Computed as 1/sqrt
        ...
        UnaryOp::Reciprocal => None, // Computed as 1/x
        UnaryOp::Tan => None,        // Computed as sin/cos
        UnaryOp::Sign => None,       // Computed via comparisons
        UnaryOp::Erf => None,        // Computed via approximation
        UnaryOp::Square => None,     // Computed as x*x
    }
}

/// Reduce operation instruction lookup.
...
pub fn render_uop(uop: &Arc<UOp>, ctx: &mut RenderContext, kernel: &mut Vec<String>) -> Option<()> {
    ...
    match uop.op() {
        // Skip meta ops - handled in pre-pass
        Op::Const(_)
        | Op::VConst { .. }
        | Op::DefineGlobal(_)
        | Op::DefineLocal(_)
        ...
        Op::Index { buffer, indices, gate } => {
            ...
            if indices.is_empty() {
                ...
                if gate.is_some() {
                    // Gated index - generate conditional
        ...
        // PointerIndex → getelementptr (like INDEX but for flat pointers)
        ...
        Op::Binary(op, lhs, rhs) => {
            ...
            // Special handling for max/min - use intrinsics
        ...
        Op::Unary(op, src) => {
            ...
            match op {
                ...
                UnaryOp::Erf => {
                    // Approximation: erf(x) using polynomial
                    // For now, use llvm.erf intrinsic if available
        ...
        // Ternary MulAcc → fmuladd intrinsic
        ...
        Op::Cast { src, dtype } => {
            ...
            // Special case: same type - just copy
            if src.dtype() == *dtype {
        ...
        // RANGE → 5-block loop structure (matches Tinygrad exactly)
        ...
        Op::End { ranges, .. } => {
            ...
            // After loop exit, emit final loads for any pending reduces
            // This gives us the accumulated value after the loop completes
        ...
        // REDUCE → accumulate into alloca (alloca created at function entry)
        Op::Reduce { src, ranges, reduce_op } => {
            let src_val = ctx.get(src);
            let dtype = ldt(&uop.dtype());

            if ranges.is_empty() {
                // No ranges - just return the source
                kernel.push(format!("  {dst} = bitcast {dtype} {src_val} to {dtype}"));
            } else {
                // The alloca was created at function entry with name %reduce_{id}
                // We need to load, accumulate, store inside the loop
        ...
        Op::Gep { vector, indices } => {
            ...
            if indices.len() == 1 {
                ...
                // Multiple indices - extract each element and build result
                render_multi_gep(&dst, vec, &vector.dtype(), indices, &out_type, kernel);
            }
            Some(())
        }

        // VECTORIZE → insertelement chain
        Op::Vectorize { elements } => {
            render_vectorize(&dst, elements, ctx, kernel);
            Some(())
        }

        // CAT → shufflevector or insertelement chain
        Op::Cat { sources } => {
            render_cat(&dst, sources, ctx, kernel);
            Some(())
        }

        // PtrCat → build vector of pointers
        Op::PtrCat { sources } => {
            render_ptrcat(&dst, sources, ctx, kernel);
            Some(())
        }

        // Contract/Unroll → pass through to source
        ...
        _ => {
            // Unknown op - emit a comment for debugging
...
/// Render binary pow using intrinsic.
fn render_binary_pow(dst: &str, lhs: &Arc<UOp>, l: &str, r: &str, ltype: &str, kernel: &mut Vec<String>) {
    if lhs.dtype().is_float() {
        ...
        // Integer power - use loop or expand
        // For now, just use float conversion
...
/// Render reduce min/max operation.
...
/// Render multi-element GEP (extract multiple elements).
fn render_multi_gep(
    dst: &str,
    vec: &str,
    vec_dtype: &DType,
    ...
    // Extract each element and build a new vector
    ...
    if indices.len() == 1 {
        ...
        // Create undef vector and insert elements
...
/// Render CAT operation (concatenate vectors).
fn render_cat(dst: &str, sources: &[Arc<UOp>], ctx: &RenderContext, kernel: &mut Vec<String>) {
    ...
    // Build insertelement chain starting from undef
    let mut out_idx = 0;
    let mut prev = "undef".to_string();
    ...
    for src in sources.iter() {
        ...
        if src_count == 1 {
            ...
            // Vector - extract and insert each element
...
/// Render PtrCat operation (concatenate pointers into vector).
fn render_ptrcat(dst: &str, sources: &[Arc<UOp>], ctx: &RenderContext, kernel: &mut Vec<String>) {
    ...
    // Start with undef inline, not as separate instruction
    let mut prev = "undef".to_string();
