...
pub fn sint_prod(values: &[SInt]) -> SInt {
    sint_aggregate(values, 1, |vals| vals.into_iter().product(), UOp::try_mul_op, "multiplication")
}
...
pub fn sint_max(values: &[SInt]) -> SInt {
    sint_aggregate(values, 0, |vals| vals.into_iter().max().unwrap(), UOp::try_max_op, "max")
}
