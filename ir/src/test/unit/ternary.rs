...
fn test_where_basic() {
    ...
    assert_eq!(
        UOp::where_op(UOp::native_const(true), UOp::native_const(1.0f32), UOp::native_const(0.0f32)).unwrap().dtype(),
        DType::Float32
    );
}
...
fn test_where_int32() {
    assert_eq!(
        UOp::where_op(UOp::native_const(false), UOp::native_const(100i32), UOp::native_const(200i32)).unwrap().dtype(),
        DType::Int32
    );
}
...
fn test_where_same_branches() {
    ...
    let result = UOp::where_op(UOp::native_const(true), value.clone(), value).unwrap();
...
fn test_where_const_true_condition() {
    ...
    assert_eq!(
        UOp::where_op(UOp::native_const(true), UOp::native_const(100i32), UOp::native_const(200i32)).unwrap().dtype(),
        DType::Int32
    );
}
...
fn test_where_nested() {
    ...
    let inner = UOp::where_op(UOp::native_const(false), UOp::native_const(2.0f32), UOp::native_const(3.0f32)).unwrap();
    let result = UOp::where_op(UOp::native_const(true), UOp::native_const(1.0f32), inner).unwrap();
...
fn test_where_with_different_dtypes() {
    ...
    assert_eq!(
        UOp::where_op(UOp::native_const(true), UOp::native_const(5i32), UOp::native_const(5.0f32)).unwrap().dtype(),
        DType::Int32
    );
}
...
fn test_where_bool_branches() {
    assert_eq!(
        UOp::where_op(UOp::native_const(false), UOp::native_const(true), UOp::native_const(false)).unwrap().dtype(),
        DType::Bool
    );
}
...
fn test_mulacc_basic() {
    ...
    assert_eq!(
        UOp::mulacc_op(UOp::native_const(2.0f32), UOp::native_const(3.0f32), UOp::native_const(4.0f32))
            .unwrap()
            .dtype(),
        DType::Float32
    );
...
fn test_mulacc_int32() {
    assert_eq!(
        UOp::mulacc_op(UOp::native_const(5i32), UOp::native_const(6i32), UOp::native_const(7i32)).unwrap().dtype(),
        DType::Int32
    );
}
...
fn test_mulacc_with_zero_multiplier() {
    assert_eq!(
        UOp::mulacc_op(UOp::native_const(0.0f32), UOp::native_const(100.0f32), UOp::native_const(5.0f32))
            .unwrap()
            .dtype(),
        DType::Float32
    );
...
fn test_mulacc_with_zero_accumulator() {
    assert_eq!(
        UOp::mulacc_op(UOp::native_const(2.0f32), UOp::native_const(3.0f32), UOp::native_const(0.0f32))
            .unwrap()
            .dtype(),
        DType::Float32
    );
...
fn test_mulacc_with_one() {
    assert_eq!(
        UOp::mulacc_op(UOp::native_const(1.0f32), UOp::native_const(5.0f32), UOp::native_const(3.0f32))
            .unwrap()
            .dtype(),
        DType::Float32
    );
...
fn test_mulacc_negative_values() {
    assert_eq!(
        UOp::mulacc_op(UOp::native_const(-2i32), UOp::native_const(3i32), UOp::native_const(10i32)).unwrap().dtype(),
        DType::Int32
    );
}
