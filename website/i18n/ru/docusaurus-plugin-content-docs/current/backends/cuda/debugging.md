...
модуля (фиксация следует вычислительной способности: 7.8 вплоть до sm_88, 8.4
на sm_89 и sm_90, от 8.6 до 8.8 по линейке Blackwell), см.
[требования](./overview.md). Информационный лог (предупреждения, спиллы
регистров) логируется на уровне `debug` под `svod_device`. Когда ядро
предассемблировал `ptxas`, JIT драйвера не происходит, и та же диагностика
приходит от самого `ptxas` — как сообщение неудавшейся компиляции.
...
Во время выполнения CUDA-тулкит нигде не *требуется* — `ptxas` используется для
предассемблирования ядер, когда он оказался установлен, — и если он есть, его
инструменты работают и с выгруженным IR:
