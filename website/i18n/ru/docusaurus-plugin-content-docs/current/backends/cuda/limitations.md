...
- Драйвер должен быть не ниже CUDA 12.0 / R525: точки входа CUDA-графов
  привязаны по их версионированным именам из 12.0. PTX ISA фиксируется флагом
  `--cuda-feature` (7.8; 8.4 для sm_89 и новее, что требует CUDA 12.4 / R550), так
  что более новый clang этот порог не поднимает.
