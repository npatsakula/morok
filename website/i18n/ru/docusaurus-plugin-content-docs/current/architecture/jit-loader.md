...
**aarch64** — 26-битные ветвления (`CALL26`, `JUMP26`), страничная адресация ADRP (`ADR_PREL_PG_HI21`), 12-битные смещения страниц с учётом размера доступа (`ADD_ABS_LO12_NC`, `LDST8/16/32/64/128_ABS_LO12_NC`).

**riscv64** — пары вызовов (`CALL`, `CALL_PLT`), PC-relative раздельная адресация с трекингом состояния (`PCREL_HI20` + `PCREL_LO12_I/S`), абсолютная (`HI20`, `LO12_I/S`), ветвления (`BRANCH`, `JAL`), данные (`32`, `64`). Хинты линкерной релаксации (`RELAX`) пропускаются.

**loongarch64** — 26-битные ветвления (`B26`), страничная раздельная адресация (`PCALA_HI20`, `PCALA_LO12`), данные (`32`, `64`). Хинты линкерной релаксации (`RELAX`) пропускаются.
...
| `--target=<arch>-none-unknown-elf` | да | нет | Bare-metal target |
...
Если clang генерирует вызов внешней функции (редко — большинство математики обрабатывается builtins), загрузчик разрешает его через `dlsym(RTLD_DEFAULT, name)` во время загрузки. Это покрывает случаи вроде `memcpy` или платформо-специфичных символов libm, которые clang может сгенерировать вместо инлайнинга.
