...
Универсальный обработчик для `Idiv` и `Mod` над Index dtype. Реализует правила из `divandmod.py:8-96` Tinygrad в порядке приоритета. Каждое правило пробуется последовательно; первое совпадение побеждает. Правилам 1-6 нужен константный знаменатель, и рекурсивный `nest_div_by_smallest_factor` завершает эту половину; `divide_by_gcd` и `factor_remainder` — идущий за ней запасной вариант, не зависящий от знаменателя.
...
### Правило 3 — nested_div

`(a % (k*c)) // c` -> `(a // c) % k` при `k > 0`. Модуль, кратный делителю, превращает внешнее деление в остаток от внутреннего частного.

**Защита**: `op == Idiv`, числитель — это `Mod(inner, modulus)`, `modulus` нацело делится на знаменатель `c`, а у полученного `k` выполняется `vmin > 0`.

**Что делает**: Переписывает деление остатка как остаток от деления, убирая один уровень вложенности.

**Пример**: `(R % 12) // 4` -> `(R // 4) % 3`

Числитель, охватывающий ровно два значения, обрабатывается уже в Правиле 4: `fold_divmod_congruence` перебирает оба знака остатка именно для того, чтобы бинарный числитель, пересекающий один период, всё же сворачивался.
...
### Правило 6 — nest_div_by_smallest_factor
...
**Пример**: `(6*a + 4*b) // 12` → `((6*a + 4*b) // 2) // 6` → `(3*a + 2*b) // 6` → `(3*a + 2*b) // 6` (затем правило 8 завершает).

Tinygrad: `divandmod.py:62-67`. Svod: `nest_div_by_smallest_factor` в `fold_divmod_general`.

### Правило 7 — divide_by_gcd

Версия Правила 5 для переменного знаменателя. Вычисляет `GCD(all_terms..., y)`, включая и числитель, и знаменатель, затем делит обе стороны. В отличие от Правила 5, работает когда знаменатель не является константой.

**Защита**: GCD нетривиален (не 1), и оба `x` и `y` точно делятся на GCD.

**Пример**: `(4*a) // (2*b)` -> `(2*a) // b`

### Правило 8 — factor_remainder
...
:::caution
Канонизация взаимодействует с итерацией фиксированной точки движка перезаписи. Если два паттерна расходятся в порядке операндов (один канонизирует, другой порождает неканонический порядок), движок может осциллировать. Все паттерны, порождающие индексы, должны соблюдать канонический порядок, иначе сработает предел безопасности движка перезаписи — стек в 500 000 записей (`REWRITE_STACK_LIMIT`).
:::
