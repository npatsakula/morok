...
:::note Origin — часть идентичности узла
При `SVOD_ORIGIN=1` узел дополнительно несёт `OriginScope`, в котором он был построен, и
этот origin входит в его content-хеш. Два идентичных подграфа, построенных в разных
скоупах, становятся *разными* узлами и не разделяются до тех пор, пока разрез на ядра не
снимет origin. Исключение — литералы, наряду с `BUFFER`/`PARAM`/`UNIQUE`: одну и ту же
константу два скоупа строят независимо, поэтому origin на ней лишь расщепил бы узел,
который разрез снова склеивает. О компромиссе см. раздел «Привязка ядер к коду модели» в
