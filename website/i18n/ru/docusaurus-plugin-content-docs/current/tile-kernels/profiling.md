...
| **3 — статическая занятость** | расход VGPR / SGPR / LDS / scratch и **занятость в %** | AMD: декодируется из дескриптора ядра. CUDA: `cuFuncGetAttribute` плюс `cuOccupancyMaxActiveBlocksPerMultiprocessor` | без диспатча — статическое декодирование на AMD, запрос к драйверу на CUDA |
...
- **Уровень 3** декодирует ресурсы AMD (VGPR/SGPR/LDS/scratch) прямо из дескриптора ядра, вообще без
  обращения к GPU. Его занятость в % смоделирована только для gfx11 (RDNA3/3.5, wave32), у которой
  геометрия регистрового файла известна; на CDNA3 (wave64) ресурсы всё равно показываются, но в
  столбце занятости стоит `-`. Это число на AMD — только ограничитель первого порядка **по VGPR**;
  пределы по LDS и по размеру рабочей группы в него не входят. На CUDA цифры берутся, наоборот, из
  загруженной функции: регистры на поток, статическая разделяемая память и локальные (scratch) байты
  из `cuFuncGetAttribute`, а занятость считает сам *драйвер*
  (`cuOccupancyMaxActiveBlocksPerMultiprocessor` для размера блока последнего запуска, относительно
  ёмкости SM по потокам), так что она уже учитывает и разделяемую память, и форму блока. Столбца
  SGPR на CUDA нет.
...
| `SVOD_ORIGIN` | любое значение, кроме пустого и `0`, записывает скоуп, в котором построена каждая операция (путь модуля, место вызова, узел ONNX), см. ниже — читается в момент построения операции в `svod-ir`, а не через `from_env` |
...
показывала matmul в десятки раз выше пика железа), так что для таких ядер в **столбце GFLOP/s стоит
...
| «Почему занятость низкая — регистры или LDS?» | столбцы VGPR/SGPR/LDS/occ% уровня 3 (замерять прогон не нужно) |
...
[AMD-бэкенд → Отладка](../backends/amd/debugging) или
[CUDA-бэкенд → Отладка](../backends/cuda/debugging).
