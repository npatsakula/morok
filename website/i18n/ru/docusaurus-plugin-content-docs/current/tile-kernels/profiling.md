...
pub struct ProfileOptions {
    ...
    pub origin_depth: Option<usize>, // глубина сводки по происхождению; None — полный путь
}
```
...
## Привязка ядер к коду модели

Имя ядра вроде `r_128_3_32_4_2_2_2_4_4_192_2` говорит о его форме, но не о том, какому слою
оно служит. С `SVOD_ORIGIN=1` каждая тензорная операция запоминает scope, в котором была
построена, — путь модуля вроде `encoder.layers.3.ffn1`, место вызова публичной операции или
индекс узла ONNX, — а планировщик переносит объединение на каждый запуск. Шестнадцать
...
Модели открывают scope-ы вдоль путей своего state-dict (`OriginScope::module`), импортёр ONNX
открывает по одному на узел, а имена стадий (`vad`, `encoder`, `ctc_head`) — метки в корне.
Рукописные ядра `tk` привязываются так же, как остальные: активный при построении ядра scope
становится его происхождением.

Если прогон несёт происхождение, `render_table()` добавляет две сводки:

- **exclusive** относит каждый запуск один раз — к основному происхождению (scope, породивший
  сохраняемое значение), поэтому строки складываются в общую сумму;
- **inclusive** относит каждый запуск к каждому предку каждого слитого в него происхождения,
  поэтому родительская строка содержит дочерние, а строки пересекаются.
...
scope-ах, больше не сливаются до разреза на ядра. Программы ядер это не затрагивает, но
вспомогательный код, который заново строит одно и то же выражение на каждом месте вызова,
должен работать под `OriginScope::suspend()` или получать уже материализованные входы.
