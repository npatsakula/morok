...
- **`Kernel`** (`tk/src/kernel.rs`) — билдер с немедленным исполнением. Он выдаёт вам сырьё:
  ...
  разделяемой памяти и регистровые буферы (и то и другое — `BUFFER`, они различаются по
  `addrspace = Local` / `Reg`) и глобальные параметры (`PARAM`). Вы привязываете к нему тензоры и
  просите у него тайлы.
- **`Group`** (`tk/src/group/`, по подмодулю на каждую задачу — `movement`, `mma`, `reduce`,
  `shuffle`, `elementwise`) — кооперирующаяся волна (или группа волн). На ней висит
...
`tk`-ядро использует `Some(vec![])`: расписание вы написали руками, поэтому оптимизатор не применяет
вообще никаких оптимизаций расписания. Общие переписывания, нужные любому ядру перед кодогенерацией
(алгебраическое упрощение, опускание dtype индексов), по телу всё же проходят; чего не случается
никогда — это перетайлинга, перевекторизации и переупорядочивания. А на уровне графа переписывания
планировщика *сохраняют вызовы* — в тело написанного руками ядра они не спускаются вовсе. Ваш
настроенный руками цикл доживает до кодогенерации ровно таким, как написан, — и при этом остаётся
обычным UOp-графом, который *тот же* рендерер превращает в LLVM IR, а *та же* среда выполнения
исполняет.
...
неверный ответ. Публичные ядра `tk` (`tk/src/kernels/`: одновыходные — через `launch_custom` в
`tk/src/launch.rs`, а многовыходные k-means и k-NN встраивают ту же политику у себя) устроены так,
чтобы это было невозможно. Каждая точка входа возвращает результат
из трёх вариантов:
...
[путь matmul в главе про IR](../architecture/ir-design). Реальное ядро добавляет `WMMA` и узлы
`BUFFER` в адресных пространствах `Local` (LDS) и `Reg` (регистры), но форма та же: SINK над STORE,
ограниченный диапазонами.
