...
(`ArchCaps::for_arch(resolve_arch(&ta.device())).wave_size`): волна — это 64 лейна на CDNA, но 32 на
RDNA и на NVIDIA, и размерность блока *и есть* это число лейнов. Выходной слайс идёт первым,
входы — вторыми, и **этот порядок — контракт**, на который опирается следующий шаг.
...
лейнов* на каждой поддерживаемой архитектуре, поэтому именование **роли** вместо зашитого фрагмента
позволяет одному телу компилироваться под все из них. Мы просим у ядра роль `Accumulator` — попросту
роль для результирующего тайла полной точности, который выдаёт и сложение, не только MMA, — а
`Kernel::frag` переадресует запрос в `ArchCaps::frag`, чтобы разрешить физический фрагмент под
целевую платформу: wave64 на CDNA, чётно-нечётная разметка wave32 на RDNA, двухполовинная разметка
`mma.sync` на CUDA.
...
`warp.load` заполняет его из глобальной. `MoveIdx::block((0, 0, 0, 0), 2)` говорит, *какой именно*
тайл глобальной читать: кортеж — координата тайла вдоль каждой из четырёх размерностей (все нули,
ведь у единственного тайла `16×16` есть лишь позиция `(0, 0)`), а `2` — ось, вдоль которой эти тайлы
...
`1`.) Волна сообща затягивает все 256 элементов прямо в регистры, уже в той разметке,
...
| **Запрашивать фрагменты по роли, а не по константе** | Именно `ker.frag(role)` заставляет одно тело работать и на wave32, и на wave64, *и* на warp32 от NVIDIA. |
...
`SINK`. **Нет** ни `Wmma`, ни `BUFFER` в адресном пространстве `Local`: это проход только через
регистры и обратно, самое компактное ядро, какое IR способен выразить.
...
Дальше — заковыка, из-за которой авторство руками по-настоящему тяжело: как удержать ядро корректным
сразу на разных размерах волн и разметках фрагментов —
