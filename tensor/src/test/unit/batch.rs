...
crate::codegen_tests! {
    fn test_batch_two_independent(config) {
        test_setup();
        let a = Tensor::full(&[4], 2.0f32, DType::Float32);
        let b = Tensor::full(&[3], 3.0f32, DType::Float32);
    ...
    fn test_batch_single(config) {
        test_setup();
        let t = Tensor::full(&[5], 7.0f32, DType::Float32);
    ...
    fn test_batch_mixed_realized(config) {
        test_setup();
        let a = Tensor::full(&[3], 1.0f32, DType::Float32);
        a.realize_with(&config).unwrap();
        let b = Tensor::full(&[3], 2.0f32, DType::Float32);
        Tensor::realize_batch_with([&a, &b], &config).unwrap();
    ...
    fn test_batch_mixed_dtypes(config) {
        test_setup();
        let a = Tensor::full(&[3], 5.0f32, DType::Float32);
    ...
    fn test_batch_three_outputs(config) {
        test_setup();
        let a = Tensor::full(&[2], 1.0f32, DType::Float32);
        let b = Tensor::full(&[3], 2.0f32, DType::Float32);
        let c = Tensor::full(&[4], 3.0f32, DType::Float32);
    ...
    fn test_batch_shared_with_reduce(config) {
        ...
        let two = Tensor::full(&[4], 2.0f32, DType::Float32);
        let doubled = (&x * &two).unwrap();
    ...
    fn test_batch_diamond_output(config) {
        ...
        let x = Tensor::from_slice([1.0f32, 2.0, 3.0]);
        let ten = Tensor::full(&[3], 10.0f32, DType::Float32);
        let a = (&x + &ten).unwrap(); // output 1
        let b = (&a * &a).unwrap(); // output 2 (consumes a)
        Tensor::realize_batch_with([&a, &b], &config).unwrap();
        assert_close_f32(&a.as_vec::<f32>().unwrap(), &[11.0, 12.0, 13.0], 1e-6);
    ...
    fn realize_batch_takes_shared_refs(config) {
        ...
        let outputs: Vec<Tensor> =
            [2.0f32, 3.0]
                .iter()
                .map(|&v| (&Tensor::from_slice([1.0f32, 2.0]) * &Tensor::full(&[2], v, DType::Float32)).unwrap())
                .collect();
    ...
    fn test_prepare_batch_execute(config) {
        test_setup();
        let a = (&Tensor::from_slice([1.0f32, 2.0]) + &Tensor::from_slice([3.0f32, 4.0])).unwrap();
        let b = (&Tensor::from_slice([10.0f32, 20.0]) * &Tensor::from_slice([2.0f32, 3.0])).unwrap();
...
/// Hash-consing folds structurally identical graphs into one node; a batch
/// holding two such tensors realizes one plan output and wires it to both.
#[test]
fn realize_batch_shares_one_output_between_identical_graphs() {
    let a = Tensor::from_slice([1.0f32, 2.0, 3.0]);
    let x = (&a * 2.0).unwrap();
    let y = (&a * 2.0).unwrap();
    assert!(std::sync::Arc::ptr_eq(&x.uop(), &y.uop()));
    Tensor::realize_batch([&x, &y]).unwrap();
    assert_eq!(x.as_vec::<f32>().unwrap(), vec![2.0, 4.0, 6.0]);
    assert_eq!(y.as_vec::<f32>().unwrap(), vec![2.0, 4.0, 6.0]);
}
