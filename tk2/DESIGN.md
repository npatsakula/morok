...
**Harness lesson (why the ceiling was misjudged): `mfmautil` is not a trustworthy ABSOLUTE.** For a fixed matmul `mfma_busy` is constant (total matrix work, = 2³⁰ for 4096³ regardless of kernel), so the old `mfma_busy/(F_peak·wall·simds)` was just `∝ 1/wall` — redundant with TFLOPS and >1 (KFD's `max_engine_clk_fcompute` is nominal, not the boost clock). Rewrote it to rocprofiler-compute's exact formula (`mfma_busy / (4·CU_per_GPU · GRBM_GUI_ACTIVE_per_XCD)`, a *measured* denominator; `runtime/src/profiler.rs`), but it **still reads >1 empirically** (tk 4096 → 2.64) ⇒ a residual counter-granularity mismatch (summed `ValuMfmaBusyCycles` over-counts vs the `4·cu_per_gpu` model by ~2.6×) to resolve against rocprofv3. **Until then: gate on measured TFLOPS vs a real reference (tk 646 / hipBLASLt), and use `mfmaduty = MFMA/SQ-busy` as the relative signal** — that one IS meaningful (tk 27 vs our 13.5 ⇒ tk fits the same MFMA work into HALF the SQ-busy cycles; our headroom is the non-MFMA overhead: addressing, LDS, barriers, waits — exactly what ping-pong + sched target).
