...
1. **`kloop` + `stages=1`.** Land the combinator; refactor `kblock_impl` onto it. **Gate: bit-exact + 385 TF preserved** (the abstraction is free) — this is the risk-free proof the structure is right before adding overlap.
2. **`stages=2`.** The prologue/steady/epilogue expansion + double-buffered LDS (2× `strip` or a rotated buffer) + the single-barrier choreography + the last-block guard (`nblocks<stages` → fall back to `stages=1`). **Gate: bit-exact; mfmautil 0.33→↑** (target: hide the fill latency, close toward tk's 0.65). The *hard part* is the ordering edges — `stage(k+1)` and `step(k)` must be edge-*independent* for overlap yet both fenced by the iter barrier, and the accumulator carry must thread prologue→steady→epilogue; the completeness verifier checks the edges (this is the WAR-safety / silent-miss class, §2.7).
3. **Hang the rest off the same combinator:** `sched_group_barrier` analytic counts inside the steady body (CK's `HotLoopScheduler`); then **warp-phase specialization** (some warps stage while others MFMA — §4's "wave ping-pong → warp-specialization pass", the fuller ping-pong).

**Note on `.apply`:** because `stages` is now a first-class field, ping-pong *could* also be an `.apply(PipelinePass)` that flips `1→2` (all generation lives in the expansion, like `SwizzlePass` flipping a layout attribute) — but it can equally stay builder-config like the warp grid. **The real decision is making the map-reduce first-class; the pass-vs-config choice is then trivial and deferrable.** New builder surface needed: `kloop` (+ its expansion), a rotated/2× LDS strip, and the runtime buffer-slot offset — no new *lowering* primitive (Range/End/After/Barrier already suffice).
