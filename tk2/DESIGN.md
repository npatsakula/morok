...
- **`stages=2`** (ping-pong) → **prologue** `stage(0)→buf[0]; barrier`; **steady** `Range(nblocks−1)` with counter `k`: `stage(k+1)→buf[(k+1)%2]` (global loads + `ds_write`, *no ordering edge* to the step below → overlaps) ‖ `step(k) from buf[k%2]` (acc read routes `[init, range, prev-step]`); ONE `barrier` per iter (fences the stage(k+1) writes for next iter + the WAR on `buf[k%2]`, covered by the 2-buffer distance); **epilogue** `step(nblocks−1) from buf[(nblocks−1)%2]` reading `frag_after(acc,[loop_end])`, then scatter. Buffer id `k%2` is a runtime `idx_mod`, so `buf` carries a runtime slot offset `slot·strip_size`.
...
2. **`stages=2` — register-staged prefetch (REVISED per the HK cdna3 dissection, below).** NOT LDS double-buffer. **Gate: bit-exact; mfmautil 0.33→↑** toward tk's 0.65. The hard part is the ordering edges (the prefetched `buffer_load`→VGPR of block k+1 stays in flight across block k's MFMAs; its `ds_write` into the *single* LDS tile is deferred past the MMA range via a WAR `After` edge; the accumulator carry threads prologue→steady→epilogue) — the WAR-safety / silent-miss class the completeness verifier checks (§2.7).
3. **Hang the rest off the same combinator:** **warp-phase ping-pong** (`warp_id/4` split + an asymmetric prologue/epilogue `s_barrier` over a wave subset — §4's warp-spec) and **schedule steering** (`s_setprio` around MFMAs + `sched_barrier(0)` fences) — the latter is the one genuinely new *codegen emission* primitive, and (per HK) the likely perf delta to ~763 TF.
