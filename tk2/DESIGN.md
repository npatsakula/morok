...
**Harness (2×2 → 128×128 tile, vec+sw, vs single-warp 64² sw-vec):** N=1024 32→42µs (mw **slower** — grid-starved: (1024/128)²=64 WGs ≪ 304 CUs), N=2048 112→100µs (−11%), **N=4096 763→521µs (−32%) = 264 TF, ~69% of tk's ~380 TF ceiling** (was 47%). **PMC: mfmautil 0.564→0.819** (the bigger tile amortises the two barriers over 4× more MFMAs → MFMA pipe 82% busy), mfmaduty 6.1→9.9, VGPR still 96 (per-warp unchanged), LDS 16→32 KB. This is the (tile, N) crossover the §5b prediction called: the 128² tile wins at large N, loses the grid-starved small-N corner — so production needs **arch/shape dispatch** (128²/256² at large N, 64²/naive below). **NEXT: (1) the residual 18% MFMA-idle is single-buffer fill latency → register-staged prefetch / double-buffer (a `PrefetchPass`, Pipelining band) to overlap block k+1's fill with block k's MFMAs; (2) 256² (wm·wn=8–16 warps, LDS-budget bound); (3) `sched_group_barrier` analytic scheduling; (4) the shape-dispatch policy.**
