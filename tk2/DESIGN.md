...
**Harness (sw-vec): N=2048 126→113µs, N=4096 886→763µs — ~180 TF, ~47% of tk's ~380 TF ceiling** (was 39%). The scalar B fill's per-element addressing was a huge sink: **VALU 404M→158M (−61%), VGPR 156→96, mfmautil 0.48→0.564, mfmaduty 4.9→6.2.** The VGPR drop to 96 (→ up to ~5 waves/SIMD by register budget) is the strategic lever for what's next. **NEXT: package gather+A-fill+B-transpose+swizzle as a `VectorizePass`** refinement (open Q: genuine rewrite-a-hole pass vs the current builder-emission — the layout knowledge lives in the builder, so a hole-based pass needs a contiguous-run marker), then spend the VGPR headroom on a **bigger tile** (128²/256²) + register-staged prefetch for occupancy/latency-hiding at low N, then `sched_group_barrier` schedule steering.
