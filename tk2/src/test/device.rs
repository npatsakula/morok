...
/// **N-run async-LDS ping-pong race detector for pipe2.** A correct deterministic kernel is
/// identical across runs, so any run whose output deviates from the f32 reference (beyond the
/// bf16 tolerance) is the single-buffer LDS race firing under the 8-wave ping-pong offset. Runs
/// pipe2 `SVOD_RUNS` times (default 64) at 4096³ and reports, per failing run, the count and the
/// first few differing `(row,col)` — localizing WHICH warp-row/slice's gather the commit clobbers.
/// The reference is one GPU f32 matmul (computed once); the race produces gross (out-of-tolerance)
/// misses, so tolerance detection is decisive. `SVOD_DEVICE=AMD:0 SVOD_RUNS=128 cargo test -p \
/// svod-tk2 --lib -- --ignored device::pipe2_race_detector --nocapture`
#[test]
#[ignore]
fn pipe2_race_detector() {
    let runs: usize = std::env::var("SVOD_RUNS").ok().and_then(|s| s.parse().ok()).unwrap_or(64);
    let sz: usize = std::env::var("SVOD_N").ok().and_then(|s| s.parse().ok()).unwrap_or(4096);
    let (m, n, k) = (sz, sz, sz);
    let dev = svod_dtype::default_device::default_device();
    let mut a = Tensor::rand_with(&[m, k], DType::BFloat16, dev.clone()).expect("rand a");
    let mut b = Tensor::rand_with(&[n, k], DType::BFloat16, dev).expect("rand b"); // B is [N,K] (A·Bᵀ)
    a.realize().expect("realize a");
    b.realize().expect("realize b");
    // Reference C = A·Bᵀ via a GPU f32 matmul over the SAME bf16-rounded operands (fast; computed once).
    let bt = b.try_transpose(0, 1).expect("Bᵀ");
    let bf = bt.cast(DType::Float32).expect("b→f32");
    let mut rr = a.cast(DType::Float32).expect("a→f32").matmul(&bf).expect("reference matmul");
    rr.realize().expect("realize reference");
    let expected = rr.as_vec::<f32>().expect("read reference");
    let atol = 0.02 * (k as f32).sqrt();

    let prog =
        crate::kernels::matmul_lds_kblock_mw_pipe2(m, n, k, 128, 64, 2, 4, 64).apply(VectorizePass).apply(SwizzlePass);
    let out = Tensor::empty(&[m, n], DType::Float32);
    let mut y = graph_kernel(prog, out, &[&a, &b]).expect("wrap pipe2");
    let plan = y.prepare().expect("prepare");

    let mut fails = 0usize;
    let mut hist: std::collections::BTreeMap<usize, usize> = std::collections::BTreeMap::new(); // col → count
    for r in 0..runs {
        plan.execute().expect("execute run");
        let got = y.as_vec::<f32>().expect("read run");
        let bad: Vec<usize> = (0..got.len())
            .filter(|&i| {
                let (g, e) = (got[i], expected[i]);
                !g.is_finite() || (g - e).abs() > atol + 2e-2 * e.abs()
            })
            .collect();
        if !bad.is_empty() {
            fails += 1;
            for &i in &bad {
                *hist.entry(i % n).or_default() += 1;
            }
            let locs: Vec<(usize, usize)> = bad.iter().take(6).map(|&i| (i / n, i % n)).collect();
            println!(
                "run {r}: {} bad elems, first locs {:?} (val {} vs {})",
                bad.len(),
                locs,
                got[bad[0]],
                expected[bad[0]]
    ...
    println!("pipe2 race detector {m}³: {fails}/{runs} runs failed");
    if !hist.is_empty() {
        let top: Vec<(usize, usize)> = {
            let mut v: Vec<_> = hist.iter().map(|(&c, &n)| (c, n)).collect();
            v.sort_by_key(|x| std::cmp::Reverse(x.1));
            v.into_iter().take(12).collect()
        };
        println!("failing columns (col → hits), top: {top:?}");
    }
    assert_eq!(fails, 0, "pipe2 non-deterministic/incorrect over {runs} runs — async-LDS ping-pong race");
}

/// **Advancing-base vs fixed-base bit-exact diff** — localizes the advancing-base addressing bug.
/// Correct advancing-base loads the SAME bf16 values as fixed-base (identical MFMA order), so the
/// f32 result must be BIT-IDENTICAL; any differing element is an advancing-base addressing defect
/// (e.g. the base-high `s_addc` sink). Builds both forms (toggling `SVOD_ADV_BASE` around each
/// construction), runs each once, and reports the differing-element count + first locations +
/// per-tile histogram (which `(tile_m,tile_n)`/warp the wrong load lands in). `SVOD_DEVICE=AMD:0 \
/// SVOD_N=4096 cargo test -p svod-tk2 --lib -- --ignored device::pipe2_adv_vs_fixed_diff --nocapture`
#[test]
#[ignore]
fn pipe2_adv_vs_fixed_diff() {
    let sz: usize = std::env::var("SVOD_N").ok().and_then(|s| s.parse().ok()).unwrap_or(4096);
    let (m, n, k) = (sz, sz, sz);
    let dev = svod_dtype::default_device::default_device();
    let mut a = Tensor::rand_with(&[m, k], DType::BFloat16, dev.clone()).expect("rand a");
    let mut b = Tensor::rand_with(&[n, k], DType::BFloat16, dev).expect("rand b");
    a.realize().expect("realize a");
    b.realize().expect("realize b");

    let run = |adv: bool| -> Vec<f32> {
        if adv {
            unsafe { std::env::set_var("SVOD_ADV_BASE", "1") };
        } else {
            unsafe { std::env::remove_var("SVOD_ADV_BASE") };
        }
        let prog = crate::kernels::matmul_lds_kblock_mw_pipe2(m, n, k, 128, 64, 2, 4, 64)
            .apply(VectorizePass)
            .apply(SwizzlePass);
        unsafe { std::env::remove_var("SVOD_ADV_BASE") };
        let out = Tensor::empty(&[m, n], DType::Float32);
        ...
        let plan = y.prepare().expect("prepare");
        plan.execute().expect("execute");
        y.as_vec::<f32>().expect("read")
    };
    // Run each form twice to separate NON-DETERMINISM (a timing race ⇒ same-form runs differ) from a
    // deterministic addressing defect (same-form runs identical, but adv≠fixed).
    let count = |x: &[f32], y: &[f32]| -> (usize, f32) {
        let mut c = 0usize;
        let mut md = 0f32;
        for i in 0..x.len() {
            if x[i].to_bits() != y[i].to_bits() {
                c += 1;
                md = md.max((x[i] - y[i]).abs());
            }
        }
        (c, md)
    };
    let fixed_a = run(false);
    let fixed_b = run(false);
    let adv_a = run(true);
    let adv_b = run(true);
    let (ff, ffm) = count(&fixed_a, &fixed_b);
    let (aa, aam) = count(&adv_a, &adv_b);
    let (af, afm) = count(&adv_a, &fixed_a);
    println!("determinism {m}³ (elements differing / {}):", fixed_a.len());
    println!("  fixed vs fixed : {ff} (max|Δ| {ffm})   ← >0 ⇒ fixed itself races");
    println!("  adv   vs adv   : {aa} (max|Δ| {aam})   ← >0 ⇒ adv races (non-deterministic)");
    println!("  adv   vs fixed : {af} (max|Δ| {afm})   ← deterministic addressing gap if aa==0");
    assert_eq!(aa, 0, "advancing base is NON-DETERMINISTIC ({aa} elems vary run-to-run) — a timing race");
    assert_eq!(af, 0, "advancing base differs from fixed base at {af} elements (deterministic)");
}
