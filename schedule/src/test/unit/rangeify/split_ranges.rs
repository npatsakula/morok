...
fn modulo_splits_every_axis_type_but_the_launch_lanes(axis_type: AxisType, splits: bool) {
    ...
    assert!(
        ranges
            .iter()
            .all(|range| matches!(range.op(), Op::Range(ops::Range { axis_type: split, .. }) if *split == axis_type)),
...
fn an_image_dtype_index_is_never_split() {
    ...
    let address = UOp::new(
        Op::Index(ops::Index { buffer: image, indices: smallvec::smallvec![range.clone()] }),
        DType::Image { kind: svod_dtype::ImageKind::Float, shape: vec![4, 2, 4] },
    );
    let store =
        UOp::new(Op::Store(ops::Store { index: address, value: UOp::native_const(1.0f32), gate: None }), DType::Void);
...
fn split_simplifies_substituted_parent_in_preopt_composition() {
    ...
    let Op::End(ops::End { computation, .. }) = sources[0].op() else { panic!("expected END") };

    assert!(
        matches!(computation.op(), Op::Range(ops::Range { end, .. })
...
fn negative_divisor_split_is_simplified_immediately() {
    ...
    let Op::Sink(ops::Sink { sources, .. }) = result.op() else { panic!("expected SINK") };
    let Op::End(ops::End { computation, ranges }) = sources[0].op() else { panic!("expected END") };
...
fn image_store_modulo_split_preserves_structural_coordinates() {
    ...
    let image = UOp::new(
        Op::Param(ops::Param { shape, arg: ParamArg::buffer(0, DType::Float32, AddrSpace::Global, None).into() }),
        DType::Float32,
    );
    let range = UOp::range_axis(UOp::index_const(8), AxisId::Renumbered(0), AxisType::Loop);
    ...
    let outer = ranges
        .iter()
        .find(|range| matches!(range.op(), Op::Range(ops::Range { end, .. }) if matches!(end.op(), Op::Const(value) if value.0 == ConstValue::Int(2))))
        .expect("outer range");
    let inner = ranges
        .iter()
        .find(|range| matches!(range.op(), Op::Range(ops::Range { end, .. }) if matches!(end.op(), Op::Const(value) if value.0 == ConstValue::Int(4))))
        .expect("inner range");
    let store = result.toposort().into_iter().find(|uop| matches!(uop.op(), Op::Store(..))).expect("image store");
    let Op::Store(ops::Store { index, .. }) = store.op() else { unreachable!() };
    let Op::Index(ops::Index { indices, .. }) = index.op() else { panic!("expected image INDEX") };
